 ...
 * including real-time (caveat: via USB this is not reliable!) console output, buffered logging for
 * later transmission, and support for timestamps in milliseconds since system start. Messages can
...
void debug_log(const char *format, ...) {
    ...
    if (timestamp_us < 10000) {
        ...
        snprintf(timestamp, sizeof(timestamp), "\033[1m[%llu us]\033[0m ",
                 (unsigned long long)timestamp_us);
    } else {
        // Otherwise, display whole milliseconds
        uint64_t timestamp_ms = timestamp_us / 1000; // Convert to milliseconds
        snprintf(timestamp, sizeof(timestamp), "\033[1m[%llu ms]\033[0m ",
    ...
    if (current_debug_mode == DEBUG_BUFFERED || current_debug_mode == DEBUG_BOTH) {
        ...
        if (debug_buffer_index < DEBUG_BUFFER_SIZE) {
            ...
            written = vsnprintf(&debug_buffer[debug_buffer_index],
                                DEBUG_BUFFER_SIZE - debug_buffer_index, format, args);
            if (written > 0)
                debug_buffer_index += written;
