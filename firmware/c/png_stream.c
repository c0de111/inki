...
static bool parse_png_header(const uint8_t* png, size_t len, png_info_t* out) {
    ...
    while (off + 12 <= len) {
        ...
        if (ctype[0]=='I' && ctype[1]=='H' && ctype[2]=='D' && ctype[3]=='R') {
            if (clen < 13) { debug_log_with_color(COLOR_RED, "[PNG] IHDR short\n"); return false; }
            ...
            if (out->bit_depth != 8 || out->compression != 0 || out->filter != 0 || out->interlace != 0) {
                debug_log_with_color(COLOR_RED, "[PNG] Unsupported IHDR: depth=%u comp=%u filter=%u interlace=%u\n",
                                     out->bit_depth, out->compression, out->filter, out->interlace);
                return false;
            }
            switch (out->color_type) {
                case 0: out->bpp = 1; break; // Gray
                case 2: out->bpp = 3; break; // RGB
                case 3: out->bpp = 1; break; // Indexed
                case 6: out->bpp = 4; break; // RGBA
                default:
            ...
            if (!saw_IHDR) { debug_log_with_color(COLOR_RED, "[PNG] PLTE before IHDR\n"); return false; }
            ...
            if (!saw_IHDR) { debug_log_with_color(COLOR_RED, "[PNG] IDAT before IHDR\n"); return false; }
            if (!out->idat_first) out->idat_first = cdata;
            out->idat_total += clen;
    ...
    if (!saw_IHDR) { debug_log_with_color(COLOR_RED, "[PNG] Missing IHDR\n"); return false; }
    if (!saw_IEND) { debug_log_with_color(COLOR_RED, "[PNG] Missing IEND\n"); return false; }
...
// Unfilter a row in-place. cur points to [filter][bytes...]; prev points to [0][bytes...] (prev row's bytes)
...
static void idat_reader_init(idat_reader_t* r, const uint8_t* png, size_t len) {
    memset(r, 0, sizeof(*r));
    r->png = png; r->len = len; r->off = 8; r->idat_ptr = NULL; r->idat_rem = 0;
}
    ...
    if (r->idat_rem == 0) {
        ...
        while (r->off + 12 <= r->len) {
            ...
            if (ctype[0]=='I' && ctype[1]=='E' && ctype[2]=='N' && ctype[3]=='D') {
                r->idat_ptr = NULL; r->idat_rem = 0; break;
            }
            r->off += 12 + clen;
        }
    }
...
bool png_stream_decode_to_flash_from_xip(const uint8_t* png, size_t png_len) {
    png_info_t info;
    if (!parse_png_header(png, png_len, &info)) return false;
    ...
    if (!prev || !cur) {
        ...
        if (prev) free(prev); if (cur) free(cur);
        return false;
    }
    ...
    tinfl_init(&infl);
    idat_reader_t reader; idat_reader_init(&reader, png, png_len);
    ...
    for (y = 0; y < rows; y++) {
        ...
        while (got < need) {
            if (out_pos == out_have) {
                ...
                size_t produced = 0;
                while (produced == 0) {
                    const uint8_t* in_ptr = NULL; size_t in_avail = 0;
                    ...
                    tinfl_status st = tinfl_decompress(&infl, in_ptr, &in_sz, outbuf, outbuf, &out_sz,
                        TINFL_FLAG_PARSE_ZLIB_HEADER | TINFL_FLAG_HAS_MORE_INPUT | TINFL_FLAG_USING_NON_WRAPPING_OUTPUT_BUF);
                    idat_reader_consume(&reader, (size_t)in_sz);
                    produced = (size_t)out_sz;
                    if (produced == 0) {
                        if (st == TINFL_STATUS_DONE) {
                            debug_log_with_color(COLOR_RED, "[PNG] Inflator finished early at row %u\n", (unsigned)y);
                            free(prev); free(cur);
                            return false;
                        }
        ...
        if (!packed) { free(prev); free(cur); return false; }
        memset(packed, 0, packed_len);
        const uint8_t* px = cur + 1;
        int shift = 6; size_t di = 0; uint8_t acc = 0;
        for (uint32_t x = 0; x < info.width; x++) {
            ...
            switch (info.color_type) {
                ...
                case 2: // RGB
                    {
                        uint8_t r = px[0], g = px[1], b = px[2]; px += 3;
                ...
                case 3: // Indexed
                    {
                        uint8_t idx = px[0]; px += 1;
                        if (idx >= (uint8_t)info.palette_len) idx = 0;
                ...
                case 6: // RGBA
                    {
                        uint8_t r = px[0], g = px[1], b = px[2]; /*a=px[3]*/ px += 4;
                        uint16_t y16 = (uint16_t)(r*30 + g*59 + b*11) / 100;
                        Y = (uint8_t)y16;
                    }
                    break;
            ...
            acc |= (uint8_t)(v << shift);
            if (shift == 0) { packed[di++] = acc; acc = 0; shift = 6; } else { shift -= 2; }
        }
        if (shift != 6) packed[di++] = acc;
        if (!weathermap_flash_append_row_2bpp(packed, di)) {
            free(packed); free(prev); free(cur);
...
bool png_stream_draw_to_paint_from_xip(const uint8_t* png, size_t png_len,
    ...
    for (uint32_t y = 0; y < info.height; y++) {
        ...
        while (got < need) {
            if (out_pos == out_have) {
                ...
                while (produced == 0) {
                    const uint8_t* in_ptr = NULL; size_t in_avail = 0;
                    idat_reader_pull(&reader, &in_ptr, &in_avail);
                    if (!in_ptr || in_avail == 0) { free(prev); free(cur); return false; }
                    size_t in_sz = (size_t)in_avail;
                    ...
                    produced = out_sz;
                    if (produced == 0) {
                        if (st == TINFL_STATUS_DONE) { free(prev); free(cur); return false; }
                        continue;
                    }
                    out_have = produced; out_pos = 0;
                }
            }
            size_t to_copy = need - got;
            size_t avail = out_have - out_pos;
        ...
        for (uint32_t x = 0; x < info.width; x++) {
            if (info.color_type == 3) {
                ...
                uint8_t r=px[0], g=px[1], b=px[2]; px+=3; uint8_t Y=(uint8_t)((r*30+g*59+b*11)/100);
                if (Y < min_y) min_y = Y;
                if (Y > max_y) max_y = Y;
            } else if (info.color_type == 6) {
                uint8_t r=px[0], g=px[1], b=px[2]; px+=4; uint8_t Y=(uint8_t)((r*30+g*59+b*11)/100);
                if (Y < min_y) min_y = Y;
                if (Y > max_y) max_y = Y;
            }
        }
    ...
    if (!simple_mode) {
        ...
        if (!err_curr || !err_next) { if (err_curr) free(err_curr); if (err_next) free(err_next); free(prev); free(cur); return false; }
    }
    ...
    for (uint32_t y2 = 0; y2 < info.height; y2++) {
        ...
        for (uint32_t x = 0; x < info.width; x++) {
            ...
            if (simple_mode) {
                if (info.color_type == 3) {
                    uint8_t idx = px[0]; px += 1;
                    // Force visible structure: scale small index ranges to 4 bins and invert brightness
                    v = (uint8_t)(idx >> 1); if (v > 3) v = 3; // 0..3 for idx 0..7
                    v = (uint8_t)(3 - v); // idx 0 -> white (GRAY4), higher idx -> darker
                } else if (info.color_type == 0) {
                    uint8_t Y = px[0]; px += 1; v = (uint8_t)(3 - (Y >> 6));
                } else if (info.color_type == 2) {
                    uint8_t r=px[0], g=px[1], b=px[2]; px+=3; uint8_t Y=(uint8_t)((r*30+g*59+b*11)/100); v=(uint8_t)(3 - (Y>>6));
                } else if (info.color_type == 6) {
                    uint8_t r=px[0], g=px[1], b=px[2]; px+=4; uint8_t Y=(uint8_t)((r*30+g*59+b*11)/100); v=(uint8_t)(3 - (Y>>6));
                } else { v = 3; }
            } else {
                // Compute source luminance Ysrc 0..255
                uint8_t Ysrc;
                switch (info.color_type) {
                    case 0: Ysrc = px[0]; px += 1; break;
                    case 2: { uint8_t r=px[0], g=px[1], b=px[2]; px+=3; Ysrc=(uint8_t)((r*30+g*59+b*11)/100); } break;
                    case 3: { uint8_t idx = px[0]; px+=1; if (idx >= (uint8_t)info.palette_len) idx=0;
                              uint8_t r = info.palette[3*idx + 0];
                              uint8_t g = info.palette[3*idx + 1];
                              uint8_t b = info.palette[3*idx + 2];
                              Ysrc = (uint8_t)((r*30 + g*59 + b*11) / 100);
                            } break;
                    case 6: { uint8_t r=px[0], g=px[1], b=px[2]; px+=4; Ysrc=(uint8_t)((r*30+g*59+b*11)/100); } break;
                    default: Ysrc = 255; break;
                }
                ...
                if (info.color_type == 3) {
                    ...
                    if (rangeY > 0) {
                        ...
                        if (rel < 0) rel = 0; if (rel > (int32_t)rangeY) rel = (int32_t)rangeY;
                ...
                if (adj < 0) adj = 0; if (adj > 255) adj = 255;
                // Quantize to nearest of 4 levels: 0,85,170,255
                v = (uint8_t)((adj + 42) / 85); if (v > 3) v = 3;
                int32_t Yq = (int32_t)v * 85;
                int32_t e = adj - Yq; // error to diffuse
                if (x + 1 < info.width) {
                    err_curr[x + 1] += (int16_t)(e * 7);
                }
                if (y2 + 1 < info.height) {
                    if (x > 0)             err_next[x - 1] += (int16_t)(e * 3);
                                            err_next[x]     += (int16_t)(e * 5);
                    if (x + 1 < info.width) err_next[x + 1] += (int16_t)(e * 1);
                }
            }
            ...
            if (dx < Paint.Width && dy < Paint.Height) {
                ...
                if (col != GRAY4) drawn_nonwhite++;
                if (v==0) lvl0++; else if (v==1) lvl1++; else if (v==2) lvl2++; else lvl3++;
            }
        }
        // Advance error buffers if in FS mode
        if (!simple_mode) {
            int16_t* tmpe = err_curr; err_curr = err_next; err_next = tmpe;
            memset(err_next, 0, info.width * sizeof(int16_t));
        }
        uint8_t* tmp = prev; prev = cur; cur = tmp;
    }

    if (err_curr) free(err_curr); if (err_next) free(err_next);
    free(prev); free(cur);
