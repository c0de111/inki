...
// flash_writer: used for LOGO uploads (small, in recv_cb - acceptable since logo is tiny)
// firmware OTA uses the ota_t state machine below instead
static struct {
...
typedef struct {
    ...
    size_t expected;                   // total firmware bytes (from Content-Length)
    uint32_t slot_offset;              // target flash slot base address
    struct tcp_pcb *tpcb;              // stored for tcp_recved + HTTP response
    int progress_pct;
} ota_t;
...
static void handle_post_firmware_update(struct tcp_pcb *tpcb, struct pbuf *p, const char *buffer,
    ...
    // Initialise upload session (type used for recv_cb routing)
    upload_session.active = true;
    upload_session.total_received = 0;
    ...
    // Timing estimate: 43ms/sector (38ms erase + 5ms sleep), 2ms/page (1ms program + 1ms sleep)
    ...
    ota.expected = upload_session.expected_length;
    ota.slot_offset = target_offset;
    ota.tpcb = tpcb;
    ota.state = OTA_ERASING;

    // Stage any body bytes that arrived in the same TCP segment as the HTTP header
    const char *body = strstr(upload_session.header_buffer, "\r\n\r\n");
    if (body) {
        body += 4;
        size_t body_len =
            upload_session.header_length - (size_t)(body - upload_session.header_buffer);
        if (body_len > ota.expected)
            body_len = ota.expected;
        if (body_len > 0) {
            // Staging buffer is empty at this point, no wrap needed
            memcpy(ota.staging, (const uint8_t *)body, body_len);
            ota.write_head = body_len;
        }
    }

    // DO NOT call tcp_recved here — flow control is tied to flash writes in the event loop
    (void)copied;
}
...
static err_t recv_cb(void *arg, struct tcp_pcb *tpcb, struct pbuf *p, err_t err) {
    ...
    if (!upload_session.header_complete) {
            ...
            if (sscanf(first_line, "%7s %63s", method_str, path_str) == 2) {
                ...
                if (method != -1) {
                    ...
                    if (route) {
                        ...
                        // Route not found
                        dlog("Route not implemented: %s %s\n", method_str, path_str);
                        cleanup_and_return(tpcb, p, copied);
                        return ERR_OK;
                    }
                }
        ...
        if (n > avail)
            n = avail; // should not happen; flow control prevents overflow
        size_t pos = ota.write_head % OTA_STAGING_SIZE;
        size_t first = OTA_STAGING_SIZE - pos;
        if (n <= first) {
            memcpy(ota.staging + pos, buffer, n);
        } else {
            memcpy(ota.staging + pos, buffer, first);
            memcpy(ota.staging, buffer + first, n - first);
        }
        ota.write_head += n;
    ...
    // UPLOAD_FIRMWARE completion is handled by webserver_ota_tick() in the event loop.
    if (upload_session.active && upload_session.type != UPLOAD_FIRMWARE &&
        upload_session.total_received >= upload_session.expected_length) {
        debug_status("OK", "Upload: complete (%d bytes)\n", (int)upload_session.total_received);
...
static void flush_page_to_flash(void) {
    ...
    restore_interrupts(ints);
    // Note: sleep_ms() here is ineffective (called inside CYW43 async_context callback,
    // lock is held, network processing deferred until recv_cb returns). Removed.
...
// =============================================================================
// OTA TICK — called from webserver_run() event loop (Thread mode)
// sleep_ms() here is effective: CYW43 IRQ can fire between page writes.
// =============================================================================

static void ota_send_response(const flash_ota_result_t *r) {
    char msg[1024];
    const firmware_header_t *h = &r->header;

    if (!r->valid) {
        debug_status("ERROR", "OTA: invalid header (magic mismatch)\n");
        send_firmware_update_page(ota.tpcb,
                                  "<h2 style='color:red'>❌ Invalid firmware header (magic)</h2>");
        return;
    }
    if (!r->slot_ok) {
        debug_status("ERROR", "OTA: slot mismatch (header slot=%u)\n", h->slot);
        send_firmware_update_page(
            ota.tpcb, "<h2 style='color:red'>❌ Slot mismatch — wrong firmware binary</h2>");
        return;
    }
    if (!r->crc_ok) {
        ...
        snprintf(msg, sizeof(msg),
                 ...
                 r->actual_crc);
        send_firmware_update_page(ota.tpcb, msg);
        return;
    }

    // Firmware is valid — check use-case meta and send success page
    bool uc_valid = firmware_header_use_case_meta_valid(h);
    ...
    if (uc_mismatch) {
        snprintf(warn_html, sizeof(warn_html),
                 ...
                 settings_zeroed ? "Use-case settings were reset to 0 and must be reconfigured."
    ...
    debug_status("OK", "OTA: firmware valid, slot %u marked\n", h->slot);
    send_firmware_update_page(ota.tpcb, msg);
}

// Read one page from the OTA staging ring buffer into dst[FLASH_PAGE_SIZE].
...
static void webserver_ota_tick(void) {
    switch (ota.state) {
    case OTA_IDLE:
        return;

    case OTA_ERASING:
        // One sector per tick; sleep_ms(5) here is effective (Thread mode).
        if (flash_ota_erase_next_sector()) {
            ota.state = OTA_WRITING;
            debug_status("OK", "OTA: erase done, receiving firmware\n");
        }
        sleep_ms(5);
        break;
    ...
    case OTA_WRITING: {
        ...
        if (pending >= FLASH_PAGE_SIZE) {
            ...
            // Open TCP window for exactly one page — browser sends no faster than we flash
            cyw43_arch_lwip_begin();
            tcp_recved(ota.tpcb, FLASH_PAGE_SIZE);
            cyw43_arch_lwip_end();

            sleep_ms(1); // effective here: CYW43 IRQ fires between page writes
            ...
            if (ota.expected > 0) {
                ...
                if (pct >= ota.progress_pct + 10) {
                    ...
                    debug_status("OK", "OTA: %d%%\n", pct);
                }
            }
        } else if (all_received) {
            // Flush last partial page (pad with 0xFF) then move to done
            if (pending > 0) {
                uint8_t page[FLASH_PAGE_SIZE];
    ...
    case OTA_DONE: {
        debug_status("OK", "OTA: validating firmware\n");
        flash_ota_result_t result = flash_ota_finish(ota.expected);
        ota_send_response(&result);
        reset_upload_session();
        upload_session.active = false;
        upload_session.header_complete = false;
        upload_session.header_length = 0;
...
// =============================================================================
// CAPTIVE PORTAL (DHCP + DNS)  — moved here from main.c
// =============================================================================
...
static void captive_start_dhcp(void) {
...
static void captive_start_dns(void) {
    struct udp_pcb *pcb = udp_new_ip_type(IPADDR_TYPE_V4);
    if (!pcb)
        return;
...
void webserver_run(void) {
    ...
    uint8_t *setup_image = epaper_init();
    if (setup_image != NULL) {
        render_page_wifi_setup(setup_image, battery_voltage);
    ...
    while (true) {
        bool ota_active = (ota.state != OTA_IDLE);
