...
static int g_idx = 0;
static int g_phase = 0; // 0=start element, 1=ON symbol, 2=OFF intra, 3=OFF letter extra, 4=OFF word extra
...
static void encode_text(const char* text, char* out, size_t out_len) {
    ...
    for (const char* p = text; *p && pos + 2 < out_len; ++p) {
        ...
        if (!code) {
            ...
            if (!first_symbol && pos < out_len - 1) out[pos++] = '/';
            first_symbol = true;
            continue;
        }
        if (!first_symbol && pos < out_len - 1) {
...
void morse_set_unit_ms(int unit_ms) { if (unit_ms > 10 && unit_ms < 2000) g_unit_ms = unit_ms; }

void morse_set_message(const char* text) {
    if (!text || !*text) text = "INKI";
...
void morse_tick(void) {
    ...
    if (!g_enabled) return;
    if (absolute_time_diff_us(get_absolute_time(), g_next) >= 0) return;
