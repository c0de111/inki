...
static uint32_t compute_crc32(const uint8_t *data, size_t len) {
    const size_t skip_header = 256; // z. B. 256 Byte Firmware-Header
    if (len <= skip_header) {
        return 0; // or error code, as needed
    }
...
int main(int argc, char **argv) {
    ...
    // Output: decimal (for scripting), or hex (your choice)
    // printf("%u\n", crc);             // decimal (used in patching)
    printf("0x%08X\n", crc); // hex (for debugging)
