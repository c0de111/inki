    ...
    if (historian_parse_timeseries(json_data, &historian_data)) {
        ...
        for (int i = 0; i < historian_data.count && i < 10; i++) {
            debug_log("[HISTORIAN] #%03d: timestamp=%" PRIu64 " ms UTC, "
                      "value=%.2f %s, state=%u\n",
                      i, historian_data.points[i].timestamp, historian_data.points[i].value,
                      historian_data.unit, historian_data.points[i].state);
        }
    } else {
...
static void draw_toggle_control(int x, int y, const sFONT *f, bool is_on) {
    ...
    if (tw < th * 2)
        tw = th * 2; // ensure minimum 2:1 space
    int right = left + tw;
 ...
 */
static void DrawSubImage(UBYTE *buffer, const SubImage *sub_image, int x, int y) {
 ...
 */
static int get_battery_level(float voltage, VoltageInterval *table, int table_size) {
 ...
 */
static void display_battery_image(float voltage, UBYTE *image_buffer, int x, int y) {
 ...
 */
static bool is_dst_europe(const ds3231_data_t *t) {
...
void format_rtc_time(const ds3231_data_t *t, char *buffer, size_t buffer_size) {
    ...
    snprintf(buffer, buffer_size, "%02i:%02i, %s, %02i. %s %04i", display_hour, t->minutes,
             get_day_of_week(t->day), t->date, get_month_name(t->month), 2000 + t->year);
}

// Output: "13:45"
static void format_short_time(const ds3231_data_t *t, char *buffer, size_t buffer_size) {
    int hour = t->hours;
    if (is_dst_europe(t)) {
        hour += 1;
        if (hour >= 24)
...
static void read_mac_address() {
    ...
    debug_log_with_color(COLOR_BOLD_GREEN, "MAC Address: %02X:%02X:%02X:%02X:%02X:%02X\n",
...
static void seatsurfing_data_received(const char *response_data, size_t length, void *arg) {
    ...
    debug_log_with_color(COLOR_GREEN, "[SEATSURFING] Received %d bytes of response data\n",
                         (int)length);
 ...
 * It also reads their states and updates the global `pushbutton` variable to reflect the
 * combination of buttons pressed. The function accommodates configurations with up to 3 pushbuttons
 * currently (possible to increase in future versions), handling variations in the number of buttons
 * across different rooms or devices.
 *
...
static void setup_and_read_pushbuttons() {
    ...
    if (device_config_flash.data.num_pushbuttons >= 3 &&
        ...
        if (!pb3)
            pushbutton += 4; // Active low
    }
    // for possible future use, maybe examples like below make sense
    // // Setup and read pushbutton 4
    // if (device_config_flash.data.num_pushbuttons == 4 && device_config_flash.data.pushbutton4_pin
    // != 0xFF) {
...
extern char __bss_end__; // NOLINT(bugprone-reserved-identifier) minimum heap ptr (end of .bss)
extern char
    __StackLimit; // NOLINT(bugprone-reserved-identifier) maximum heap ptr (stack region below)
...
void get_memory_info(memory_info_t *out) {
    ...
    if (heap_end >= heap_base)
        heap_used = (size_t)(heap_end - heap_base);
    if (heap_limit >= heap_end)
        heap_headroom = (size_t)(heap_limit - heap_end);
    ...
    if (sp_reg >= heap_end)
        stack_margin = (size_t)(sp_reg - heap_end);
...
/**
 * Ensures the circuit remains powered by driving n-transistor (MMBT3904) -> MOSFET (TSM260P02) gate
 * -> power up for running through programme, until switched off by the programme at the end
 *
 ...
 */
static ds3231_t init_clock(void) {
    ds3231_t clock_rtc;

    // Initialize the DS3231 struct
    ds3231_init(&clock_rtc, i2c_default, DS3231_DEVICE_ADRESS, AT24C32_EEPROM_ADRESS_0);
    ...
    i2c_init(clock_rtc.i2c, I2C_FREQ); // Set frequency to 400 kHz
...
void set_rtc_from_display_string(ds3231_t *ds3231, const char *line) {
    ...
    ds3231_data_t new_time = {.seconds = 0,
                              .minutes = minute,
                              .hours = hour,
...
void set_alarmclock_and_powerdown(ds3231_t *ds3231) {
    ...
    // int total_minutes = local_hour * 60 + local_minute +
    // device_config_flash.data.refresh_minutes;
    int refresh = device_config_flash.data
                      .refresh_minutes_by_pushbutton[pushbutton & 0x07]; // use only 3 bits
...
static bool draw_flash_logo(UBYTE *buffer, int x, int y) {
 ...
 */
static void render_4gray_test_pattern(void) {
    // Title
    ...
    // Final mapping: GRAY1=0x00=Black, GRAY2=0x01=Dark Gray, GRAY3=0x02=Light Gray,
    // GRAY4=0x03=White
    ...
    // Footer
    // Paint_DrawString_EN(50, 270, "4Gray validation complete", &font_ubuntu_mono_12pt, GRAY4,
    // GRAY1);
}
 ...
 */
static void render_temperature_graph(UBYTE *image_buffer, int x, int y, int width, int height) {
...
static void render_page_fallback(int pushbutton, ds3231_t *clock, UBYTE *image_buffer,
                                 float battery_voltage);
static void render_page_wifi_setup(UBYTE *image);
static void render_page_wifisetup_seatsurfing(ds3231_t *clock, UBYTE *image_buffer,
                                              float battery_voltage);
...
static void render_page_placeholder_homematic(ds3231_t *clock, UBYTE *image_buffer,
                                              float battery_voltage) {
    (void)battery_voltage;
    ...
    Paint_DrawString_EN(page_x, title_y + title_font->Height + 8, page_line, (sFONT *)info_font,
                        WHITE, BLACK);

    int msg_x = center_x - (int)strlen(msg) * info_font->Width / 2;
    Paint_DrawString_EN(msg_x, title_y + title_font->Height + info_font->Height + 16, msg,
                        (sFONT *)info_font, WHITE, BLACK);
...
static void render_page_wifisetup_homematic(ds3231_t *clock, UBYTE *image_buffer,
                                            float battery_voltage) {
    (void)clock;
    (void)battery_voltage;
...
static void render_page_placeholder_historian(ds3231_t *clock, UBYTE *image_buffer,
                                              float battery_voltage) {
    (void)clock;
    (void)battery_voltage;
    ...
    if (device_config_flash.data.epapertype == EPAPER_WAVESHARE_7IN5_V2) {
        Paint_DrawString_EN(center_x_75 - 100, center_y_75 - 40, title, (sFONT *)font, WHITE,
                            BLACK);
        Paint_DrawString_EN(center_x_75 - 60, center_y_75 + 10, page_line, (sFONT *)font, WHITE,
                            BLACK);
        ...
        Paint_DrawString_EN(center_x_42 - 40, center_y_42 + 0, page_line, (sFONT *)font, WHITE,
                            BLACK);
...
static void render_page_wifisetup_historian(ds3231_t *clock, UBYTE *image_buffer,
                                            float battery_voltage) {
    (void)clock;
    (void)battery_voltage;
    render_page_wifi_setup(image_buffer);
...
// Render the default page with usecase-specific information and QR codes if enabled. This is the
// page without any user interaction
static void render_page_0(ds3231_t *clock, UBYTE *image_buffer, float battery_voltage) {
    ...
    if (device_config_flash.data.type == ROOM_TYPE_OFFICE &&
        ...
        Paint_DrawString_EN(40, 50, device_config_flash.data.roomname, &font_ubuntu_mono_28pt_bold,
                            WHITE, BLACK);
        if (!draw_flash_logo(image_buffer, EPD_7IN5_V2_WIDTH - inki_octopus_100_95.width - 20,
                             20)) {
            DrawSubImage(image_buffer, &inki_octopus_100_95,
                         EPD_7IN5_V2_WIDTH - inki_octopus_100_95.width - 20, 20);
        }
...
#define DRAW_SEAT(xpos, ypos, seat_ref, name_font, status_font)                                    \
    do {                                                                                           \
        ...
        if (seat_count == 1) {
            ...
            DRAW_SEAT(220, 260, seatsurfing_data[0], &font_ubuntu_mono_14pt,
                      &font_ubuntu_mono_20pt_bold);
        } else if (seat_count == 2) {
            // two seats: left/top and right/top
            DRAW_SEAT(40, 190, seatsurfing_data[1], &font_ubuntu_mono_14pt,
                      &font_ubuntu_mono_20pt_bold);
            DRAW_SEAT(400, 190, seatsurfing_data[0], &font_ubuntu_mono_14pt,
                      &font_ubuntu_mono_20pt_bold);
        } else { // seat_count >= 3
            DRAW_SEAT(40, 190, seatsurfing_data[2], &font_ubuntu_mono_14pt,
                      &font_ubuntu_mono_20pt_bold);
            DRAW_SEAT(400, 190, seatsurfing_data[1], &font_ubuntu_mono_14pt,
                      &font_ubuntu_mono_20pt_bold);
            DRAW_SEAT(220, 320, seatsurfing_data[0], &font_ubuntu_mono_14pt,
                      &font_ubuntu_mono_20pt_bold);
        }
...
#undef DRAW_SEAT
        ...
        Paint_DrawString_EN(70, 60, device_config_flash.data.roomname, &font_ubuntu_mono_28pt_bold,
                            WHITE, BLACK);
    }
        ...
        Paint_DrawString_EN(20, 40, device_config_flash.data.roomname, &font_ubuntu_mono_11pt_bold,
                            WHITE, BLACK);
 ...
 */
static void render_page_1(ds3231_t *clock, UBYTE *image_buffer, float battery_voltage) {
    ...
    if (device_config_flash.data.epapertype == EPAPER_WAVESHARE_7IN5_V2) {
        ...
        // Room name
        Paint_DrawString_EN(50, 60, device_config_flash.data.roomname, &font_ubuntu_mono_24pt_bold,
                            WHITE, BLACK);

        // Main message
        Paint_DrawString_EN(80, 180, "Videoconference", &font_ubuntu_mono_28pt_bold, WHITE, BLACK);
        Paint_DrawString_EN(80, 270, "Please do not disturb", &font_ubuntu_mono_20pt_bold, WHITE,
                            BLACK);
        ...
        Paint_DrawString_EN(20, 40, device_config_flash.data.roomname, &font_ubuntu_mono_18pt_bold,
                            WHITE, BLACK);
        ...
        // Format and display the current start time
        char time_string[8];
        format_short_time(&ds3231_data, time_string, sizeof(time_string));
        snprintf(buffer, sizeof(buffer), "Start: %s", time_string);
 ...
 *
 * @param room  The room configuration containing ePaper and layout settings.
 * @param clock A pointer to the initialized RTC (ds3231) structure (not used here but kept for
 * consistency).
 */
static void render_page_2(ds3231_t *clock, UBYTE *image_buffer, float battery_voltage) {
    char buffer[128]; // Buffer for formatted strings
    ...
    if (device_config_flash.data.epapertype == EPAPER_WAVESHARE_7IN5_V2) {
        ...
        Paint_DrawString_EN(70, 50, device_config_flash.data.roomname, &font_ubuntu_mono_20pt_bold,
                            WHITE, BLACK); // Display room name
 ...
 * @param clock A pointer to the initialized RTC (ds3231) structure.
 */
static void render_page_3(ds3231_t *clock, UBYTE *image_buffer, float battery_voltage) {
    ds3231_data_t ds3231_data;
    ...
    if (device_config_flash.data.epapertype == EPAPER_WAVESHARE_7IN5_V2) {
        ...
        Paint_DrawString_EN(10, 20, device_config_flash.data.roomname, &font_ubuntu_mono_14pt_bold,
                            WHITE, BLACK);

        sprintf(buffer, "ssid: %s", wifi_config_flash.ssid);
        Paint_DrawString_EN(10, 70, buffer, &font_ubuntu_mono_6pt, WHITE, BLACK);

        sprintf(buffer, "wifi_reconnect_minutes: %i",
        ...
        sprintf(buffer, "refresh_minutes: [%d,%d,%d,%d,%d,%d,%d,%d]",
        ...
        char buffer2[256];
        snprintf(buffer, sizeof(buffer), "%02i:%02i, %s, %02i. %s %04i", ds3231_data.hours,
                 ds3231_data.minutes, get_day_of_week(ds3231_data.day), ds3231_data.date,
                 get_month_name(ds3231_data.month), 2000 + ds3231_data.year);
        ...
        read_mac_address();
        sprintf(buffer, "MAC address: %02X:%02X:%02X:%02X:%02X:%02X", mac_address[0] & 0xFF,
                mac_address[1] & 0xFF, mac_address[2] & 0xFF, mac_address[3] & 0xFF,
...
// Howto page
static void render_page_4(ds3231_t *clock, UBYTE *image_buffer, float battery_voltage) {
    ...
    if (device_config_flash.data.epapertype == EPAPER_WAVESHARE_7IN5_V2) {
        ...
        int tempx = 5;
        // Title
        Paint_DrawString_EN(10, 10, "How to select pages", &font_ubuntu_mono_12pt_bold, WHITE,
                            BLACK);

        Paint_DrawString_EN(10, 45, "Hold buttons 1-3 to select a page (1-7)",
                            &font_ubuntu_mono_6pt, WHITE, BLACK);
        Paint_DrawString_EN(10, 63, "and press 'Start' at the same time", &font_ubuntu_mono_6pt,
                            WHITE, BLACK);

        Paint_DrawString_EN(15, 85 + tempx, "page #0: 'default: room occupation'",
                            &font_ubuntu_mono_6pt, WHITE, BLACK);
        Paint_DrawString_EN(15, 101 + tempx, "page #1: 'do not disturb'", &font_ubuntu_mono_6pt,
                            WHITE, BLACK);
        Paint_DrawString_EN(15, 117 + tempx, "page #2: 'universal decision maker'",
                            &font_ubuntu_mono_6pt, WHITE, BLACK);
        Paint_DrawString_EN(15, 133 + tempx, "page #3: 'display settings'", &font_ubuntu_mono_6pt,
                            WHITE, BLACK);
        Paint_DrawString_EN(15, 149 + tempx, "page #4: 'How to show pages'", &font_ubuntu_mono_6pt,
                            WHITE, BLACK);
        Paint_DrawString_EN(15, 165 + tempx, "page #5: 'set clock with server time'",
                            &font_ubuntu_mono_6pt, WHITE, BLACK);
        Paint_DrawString_EN(15, 181 + tempx, "page #6: 'not used'", &font_ubuntu_mono_6pt, WHITE,
                            BLACK);
        Paint_DrawString_EN(15, 197 + tempx, "page #7: 'not used'", &font_ubuntu_mono_6pt, WHITE,
                            BLACK);
        ...
        const int arrow_heights[] = {17, 40, 40, 40}; // PB0 shorter
        ...
        for (int i = 0; i < 4; i++) {
            ...
            // Arrowhead
            Paint_DrawLine(x, arrow_bottom - 3, x - 3, arrow_bottom - 6, BLACK, DOT_PIXEL_2X2,
                           LINE_STYLE_SOLID);
            Paint_DrawLine(x, arrow_bottom - 3, x + 3, arrow_bottom - 6, BLACK, DOT_PIXEL_2X2,
                           LINE_STYLE_SOLID);
        }
        Paint_DrawString_EN(80, 260, "Start", &font_ubuntu_mono_8pt_bold, WHITE, BLACK);
        Paint_DrawString_EN(4, 240, " Selected page # = 1  +  2  +  4 ", &font_ubuntu_mono_6pt,
                            WHITE, BLACK);
...
static void render_page_5(ds3231_t *clock, UBYTE *image_buffer, float battery_voltage) {
...
static void render_page_6(ds3231_t *clock, UBYTE *image_buffer, float battery_voltage) {
...
static void render_page_7(ds3231_t *clock, UBYTE *image_buffer, float battery_voltage) {
...
static void render_page(int pushbutton, ds3231_t *clock, UBYTE *image_buffer,
                        float battery_voltage) {
    ...
    static const page_renderer_t weathermap_pages[8] = {
        render_page_0, render_page_1, render_page_2, render_page_3,
        render_page_4, render_page_5, render_page_6, render_page_7,
    };
 ...
 */
static void render_firmware_info(float battery_voltage) {
 ...
 */
static bool is_wifi_required(int pushbutton) {
    ...
    if (pushbutton == 2) {
        debug_log(
            "Wi-Fi not required: Page 2, static information page, unviversal decision maker.\n");
        return false;
    }
 ...
 */
static void render_page_server_error(ds3231_t *clock, UBYTE *image_buffer) {
    char buffer[256]; // Buffer for formatted strings
    ds3231_data_t ds3231_data;
    ...
    if (device_config_flash.data.epapertype == EPAPER_WAVESHARE_7IN5_V2) {
        ...
        Paint_DrawString_EN(50, 350, "Please check the server status.", &font_ubuntu_mono_12pt,
                            WHITE, BLACK);
        ...
        // Paint_DrawString_EN(20, 40, device_config_flash.data.roomname,
        // &font_ubuntu_mono_18pt_bold, WHITE, BLACK);
 ...
 * @param clock Pointer to the ds3231_t structure for accessing RTC data.
 */
static void render_page_wifi_error(ds3231_t *clock, UBYTE *image_buffer) {
    char buffer[256]; // Buffer for formatted strings
    ds3231_data_t ds3231_data;
    ...
    if (device_config_flash.data.epapertype == EPAPER_WAVESHARE_7IN5_V2) {
        ...
        // Display a tip or diagnostic message
        Paint_DrawString_EN(50, 350, "Please check the WiFi settings.", &font_ubuntu_mono_12pt,
                            WHITE, BLACK);
        ...
        // Display a tip or diagnostic message
        // Paint_DrawString_EN(20, 190, "Please check the Wi-Fi settings.", &font_ubuntu_mono_9pt,
        // WHITE, BLACK);
...
static void render_page_wifi_setup(UBYTE *image) {
    int x_offset = 0;
    int y_offset = 0;
    const char *setup_url = "http://inki-setup";
    const char *setup_ip_url = "http://192.168.4.1";
    if (device_config_flash.data.epapertype == EPAPER_WAVESHARE_7IN5_V2) {
    ...
    int center_x = (EPD_4IN2_V2_WIDTH / 2) + x_offset;
    int setup_url_x = center_x - ((int)strlen(setup_url) * font_ubuntu_mono_12pt_bold.Width) / 2;
    int setup_ip_url_x =
        center_x - ((int)strlen(setup_ip_url) * font_ubuntu_mono_12pt_bold.Width) / 2;

    Paint_DrawString_EN(20 + x_offset, 20 + y_offset, "WIFI Setup Mode", &font_ubuntu_mono_11pt,
                        WHITE, BLACK);
    Paint_DrawString_EN(20 + x_offset, 80 + y_offset, "Connect to ", &font_ubuntu_mono_10pt, WHITE,
                        BLACK);
    Paint_DrawString_EN(setup_url_x, 130 + y_offset, setup_url, &font_ubuntu_mono_12pt_bold, WHITE,
                        BLACK);

    Paint_DrawString_EN(20 + x_offset, 180 + y_offset, "or", &font_ubuntu_mono_10pt, WHITE, BLACK);
    Paint_DrawString_EN(setup_ip_url_x, 230 + y_offset, setup_ip_url, &font_ubuntu_mono_12pt_bold,
                        WHITE, BLACK);
    ...
    // Paint_DrawString_EN(20 + x_offset, 240 + y_offset, line1, &font_ubuntu_mono_8pt, WHITE,
    // BLACK); Paint_DrawString_EN(20 + x_offset, 250 + y_offset, line2, &font_ubuntu_mono_8pt,
    // WHITE, BLACK);
}
...
static void render_page_wifisetup_seatsurfing(ds3231_t *clock, UBYTE *image_buffer,
    ...
    int logo_x =
        (is_epaper_75 ? EPD_7IN5_V2_WIDTH : EPD_4IN2_V2_WIDTH) - inki_octopus_100_95.width - 20;
    if (logo_x < 0)
        logo_x = 0;
    if (!draw_flash_logo(image_buffer, logo_x, 10)) {
        DrawSubImage(image_buffer, &inki_octopus_100_95, logo_x, 10);
    ...
    int setup_url_x = center_x - ((int)strlen(setup_url) * font_ubuntu_mono_18pt_bold.Width) / 2;
    int setup_ip_url_x =
        center_x - ((int)strlen(setup_ip_url) * font_ubuntu_mono_18pt_bold.Width) / 2;

    Paint_DrawString_EN(center_x - 140 + x_offset, 40 + y_offset, "Wi-Fi Setup / Web UI",
                        &font_ubuntu_mono_18pt_bold, WHITE, BLACK);
    Paint_DrawString_EN(center_x - 110 + x_offset, 90 + y_offset,
                        "Connect to AP:", &font_ubuntu_mono_14pt, WHITE, BLACK);
    Paint_DrawString_EN(setup_url_x, 120 + y_offset, setup_url, &font_ubuntu_mono_18pt_bold, WHITE,
                        BLACK);

    Paint_DrawString_EN(center_x - 110 + x_offset, 170 + y_offset, "or", &font_ubuntu_mono_14pt,
                        WHITE, BLACK);
    Paint_DrawString_EN(setup_ip_url_x, 200 + y_offset, setup_ip_url, &font_ubuntu_mono_18pt_bold,
                        WHITE, BLACK);

    // Slot hint
    Paint_DrawString_EN(center_x - 150 + x_offset, 250 + y_offset,
                        "Upload opposite slot image:", &font_ubuntu_mono_12pt, WHITE, BLACK);
    Paint_DrawString_EN(center_x - 150 + x_offset, 280 + y_offset,
                        "active slot 0 -> upload inki_slot1.bin", &font_ubuntu_mono_10pt, WHITE,
                        BLACK);
    Paint_DrawString_EN(center_x - 150 + x_offset, 300 + y_offset,
                        "active slot 1 -> upload inki_slot0.bin", &font_ubuntu_mono_10pt, WHITE,
                        BLACK);
}
...
static const uint8_t dhcp_offer_template[] = {
    ...
    6, 4, 192, 168, 4, 1,             // DNS
    15, 4, 'i', 'n', 'k', 'i',        // Domain name
    119, 5, 4, 'i', 'n', 'k', 'i', 0, // Domain search list: inki
    255                               // End option
};
...
static bool dns_read_qname_lower(const uint8_t *packet, size_t packet_len, size_t *pos, char *out,
                                 size_t out_len) {
    if (!packet || !pos || !out || out_len < 2 || *pos >= packet_len) {
        return false;
    }
    ...
    while (in < packet_len) {
        ...
        // Compression pointers are not expected in QNAME for setup DNS queries.
        if ((label_len & 0xC0) != 0 || in + label_len > packet_len) {
            return false;
        }
...
static void dns_recv_cb(void *arg, struct udp_pcb *pcb, struct pbuf *p, const ip_addr_t *addr,
    ...
    size_t qname_pos = 12;
    char qname[128];
    if (!dns_read_qname_lower(req, req_len, &qname_pos, qname, sizeof(qname))) {
        pbuf_free(p);
        return;
    }

    if (qname_pos + 4 > req_len) {
        pbuf_free(p);
        return;
    }

    uint16_t qtype = (uint16_t)((req[qname_pos] << 8) | req[qname_pos + 1]);
    uint16_t qclass = (uint16_t)((req[qname_pos + 2] << 8) | req[qname_pos + 3]);
    size_t question_end = qname_pos + 4;

    bool name_match = (strcmp(qname, "inki-setup") == 0 || strcmp(qname, "inki-setup.local") == 0 ||
                       (strncmp(qname, "inki-setup.", 11) == 0 && qname[11] != '\0'));
    bool type_match = (qtype == 1 || qtype == 255);    // A or ANY
    bool class_match = (qclass == 1 || qclass == 255); // IN or ANY
    bool add_answer = (name_match && type_match && class_match);
    uint16_t rcode = name_match ? 0u : 3u; // NOERROR or NXDOMAIN

    // DNS response = original header/question + optional single A answer (192.168.4.1)
    const size_t answer_len = 16;
    if (question_end + (add_answer ? answer_len : 0) > sizeof(req)) {
        pbuf_free(p);
        return;
    }
    ...
    // Preserve RD flag from query; set QR + AA + response code.
    uint16_t resp_flags = (uint16_t)(0x8400u | (flags & 0x0100u) | (rcode & 0x000Fu));
    resp[2] = (uint8_t)((resp_flags >> 8) & 0xFF);
    resp[3] = (uint8_t)(resp_flags & 0xFF);
    resp[4] = 0x00;
    resp[5] = 0x01; // One question (handled)
    resp[6] = 0x00;
    resp[7] = add_answer ? 0x01 : 0x00; // Answer count
    resp[8] = 0x00;
    resp[9] = 0x00; // NSCOUNT
    resp[10] = 0x00;
    resp[11] = 0x00; // ARCOUNT

    size_t off = question_end;
    if (add_answer) {
        resp[off++] = 0xC0;
        resp[off++] = 0x0C; // Name pointer to QNAME at offset 12
        resp[off++] = 0x00;
        resp[off++] = 0x01; // TYPE A
        resp[off++] = 0x00;
        resp[off++] = 0x01; // CLASS IN
        resp[off++] = 0x00;
        resp[off++] = 0x00;
        resp[off++] = 0x00;
        resp[off++] = 0x3C; // TTL 60s
        resp[off++] = 0x00;
        resp[off++] = 0x04; // RDLENGTH
        resp[off++] = 192;
        resp[off++] = 168;
        resp[off++] = 4;
        resp[off++] = 1; // RDATA
    ...
    struct pbuf *resp_buf = pbuf_alloc(PBUF_TRANSPORT, (u16_t)off, PBUF_RAM);
    if (!resp_buf) {
        pbuf_free(p);
        return;
    }

    memcpy(resp_buf->payload, resp, off);
    udp_sendto(pcb, resp_buf, addr, port);
...
static void enter_wifi_setup_mode(ds3231_t *clock) {
    ...
    if (cyw43_wifi_get_mac(&cyw43_state, CYW43_ITF_AP, mac_address) != 0) {
        ...
        debug_log_with_color(COLOR_BOLD_GREEN, "AP MAC Address: %02X:%02X:%02X:%02X:%02X:%02X\n",
                             mac_address[0], mac_address[1], mac_address[2], mac_address[3],
                             mac_address[4], mac_address[5]);
    }
    ...
    debug_log_with_color(COLOR_GREEN,
                         "Setup DNS active: http://inki-setup and http://inki-setup.local -> "
                         "192.168.4.1\n");
    ...
    while (true) {
        ...
        // LED behavior: during firmware upload keep LEDs solid ON (no Morse) to avoid timing
        // side-effects and signal activity
#if LED_MORSE_ENABLED
        ...
        if (absolute_time_diff_us(get_absolute_time(), shutdown_time) < 0) {
            debug_log_with_color(COLOR_BOLD_YELLOW,
                                 "Setup timeout reached - device is shutting down.\n");
            cyw43_arch_deinit();
            transmit_debug_logs();
            set_alarmclock_and_powerdown(clock);
            exit(0);
...
static void print_firmware_slots_status(void) {
...
static bool wait_for_usb_connection(uint32_t timeout_ms) {
    ...
    while (!stdio_usb_connected() && waited_ms < timeout_ms) {
        sleep_ms(10);
        waited_ms += 10;
    }
...
int main(void) {
...
#if INKI_DEBUG_USB_WAIT
    if (wait_for_usb_connection(2500)) {
    ...
    if (pushbutton == 4) {
        debug_log_with_color(COLOR_BOLD_YELLOW,
                             "Seatsurfing: launching web interface (pushbutton 4)\n");
        enter_wifi_setup_mode(&ds3231);
    }
#endif
    ...
    if (pushbutton == 7) {
        ...
        //   return 0;  // The device will shut down inside setup mode (after timeout or user
        //   action)
    }
    ...
    if (device_config_flash.data.epapertype == EPAPER_NONE) {
        debug_log_with_color(COLOR_BOLD_YELLOW,
                             "No ePaper configured — entering WiFi setup mode\n");
        enter_wifi_setup_mode(&ds3231);
    }
    ...
    if (is_wifi_required(pushbutton)) {
        ...
        wifi_result = wifi_server_communication(battery_voltage, coin_cell_voltage);
#elif defined(USE_CASE_SEATSURFING)
    ...
    if (BlackImage == NULL) {
        ...
        // If no ePaper is configured or init failed, fall back to WiFi setup to allow
        // configuration.
