...
static void http_set_after_close(void (*cb)(void *), void *arg) {
    g_after_close_cb = cb;
...
transfer_complete:
    ...
    debug_log_with_color(COLOR_GREEN, "[HTTP] Transfer complete: %d bytes received\n",
 ...
 * 3. **HTTP Transfer**: Robust HTTP client handles connection, headers, and body streaming (no
 * chunked TE)
...
static void inki_monitor_send_after_query(float battery_voltage, float coin_cell_voltage,
                                          bool query_ok) {
...
static void homematic_single_completion(const char *body, size_t length, bool success, void *arg) {
    ...
    if (g_hm_seq.phase == HM_PHASE_UNIT) {
        ...
        char unit[8] = {0};
        if (success && body && length > 0) {
            // Find parameter block: <member><name>KEY</name><value><struct> ...
            // </struct></value></member>
            ...
            if (!hit && strcmp(key, "ACTUAL_TEMPERATURE") == 0) {
                ...
                if (hit) {
                    debug_log_with_color(
                        COLOR_YELLOW,
                        "[HOMEMATIC] Using alias TEMPERATURE for ACTUAL_TEMPERATURE (idx=%u)\n",
                        idx);
                }
            }
            if (hit) {
                ...
                // Find the <value> and then the <struct> within this member (allow
                // whitespace/newlines)
                ...
                if (val_tag && val_tag < member_end) {
                    ...
                    if (struct_tag && struct_tag < member_end) {
                        ...
                        if (u && u < member_end) {
                            ...
                            debug_log_with_color(
                                COLOR_YELLOW,
                                "[HOMEMATIC] UNIT not present in member for key %s (idx=%u)\n", key,
                                idx);
                        }
                    } else {
                        debug_log_with_color(
                            COLOR_YELLOW,
                            "[HOMEMATIC] struct not present in member for key %s (idx=%u)\n", key,
                            idx);
                    }
                } else {
                    debug_log_with_color(COLOR_YELLOW,
                                         "[HOMEMATIC] value tag not present for key %s (idx=%u)\n",
                                         key, idx);
                }
            } else {
                debug_log_with_color(
                    COLOR_YELLOW,
                    "[HOMEMATIC] Key '%s' not found in ParamsetDescription (idx=%u)\n", key, idx);
            }
        }
        // Cache and notify
        ...
        if (unit[0]) {
            debug_log_with_color(COLOR_GREEN, "[HOMEMATIC] UNIT idx=%u key=%s = '%s'\n", idx,
                                 g_hm_seq.eff.data.items[idx].key, unit);
        } else {
    ...
    if (!success || !body || length == 0 || strstr(body, "<fault>")) {
        if (g_hm_seq.attempts[idx] < 1) {
            ...
            retry = true;
            debug_log_with_color(COLOR_YELLOW,
                                 "[HOMEMATIC] Scheduling retry for idx=%u (attempt %u)\n", idx,
                                 g_hm_seq.attempts[idx]);
        } else {
    ...
    // Mark that we are ready to move on (retry same, next phase, or next idx) after TCP fully
    // closed Decide next index and phase
...
static void homematic_on_closed(void *arg) {
    ...
    // Schedule next step if there are remaining items, or if we need to run the single terminal
    // service query
...
static bool homematic_make_request(void) {
    ...
    if (data_callback)
        data_callback(NULL, 0, (void *)(uintptr_t)0xFFFF);
 ...
 * @param battery_voltage Battery voltage (for telemetry)
 * @param coin_cell_voltage Coin-cell voltage (for telemetry)
...
WifiResult wifi_server_communication(float battery_voltage, float coin_cell_voltage) {
    ...
    if (!historian_make_request()) {
        inki_monitor_send_after_query(battery_voltage, coin_cell_voltage, false);
    ...
    inki_monitor_send_after_query(battery_voltage, coin_cell_voltage, true);
