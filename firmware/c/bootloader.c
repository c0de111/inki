...
static void __noinline jump_to_firmware(uint32_t header_offset) {
    asm volatile(
        ...
        :
        : [vector] "r"(XIP_BASE + header_offset + VECTOR_TABLE_OFFSET), [vtor] "X"(
                                                                            PPB_BASE +
                                                                            M0PLUS_VTOR_OFFSET)
        : "r0", "r1");
...
static bool parse_git_version(const char *str, parsed_version_t *out) {
    unsigned int maj = 0, min = 0, pat = 0, bld = 0;
    ...
    matched = sscanf(str, "v%u.%u.%u", &maj, &min, &pat);
    if (matched == 3) {
        out->major = (uint8_t)maj;
        out->minor = (uint8_t)min;
        out->patch = (uint8_t)pat;
...
static inline bool is_valid_firmware(const firmware_header_t *hdr) {
    return hdr->valid_flag == 1 && memcmp(hdr->magic, FIRMWARE_MAGIC, FIRMWARE_MAGIC_LEN) == 0;
}

/* ───────────── Firmware Selection ───────────── */
static void bootloader_select_firmware(void) {
    const firmware_header_t *slot0 =
        (const firmware_header_t *)FLASH_PTR(FIRMWARE_SLOT0_FLASH_OFFSET);
    const firmware_header_t *slot1 =
        (const firmware_header_t *)FLASH_PTR(FIRMWARE_SLOT1_FLASH_OFFSET);
    ...
    fflush(stdout); // <- Flush USB output safely
#endif
...
// v0.build, ok0); printf("[DEBUG] Parsed slot 1: %d.%d.%d build %u (ok=%d)\n", v1.major, v1.minor,
// v1.patch, v1.build, ok1);
#endif
        if (ok0 && ok1) {
