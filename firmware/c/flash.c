...
void flash_log_status(void) {
    ...
    for (uint8_t slot = 0; slot < 2; slot++) {
        char build[16] = {0}, ver[32] = {0};
        uint32_t size = 0, crc = 0;
        uint8_t idx = 0, valid = 0;
        bool has = get_firmware_slot_info(slot, build, ver, &size, &crc, &idx, &valid);
        if (has) {
...
static struct {
    ...
    size_t total_size;     // expected firmware bytes (for CRC)
} s_ota;
...
flash_ota_result_t flash_ota_finish(size_t expected_bytes) {
    ...
    // CRC skips the 256-byte header (same as crc32_calculate in webserver_utils.c).
    // The build system computes CRC over firmware_size bytes starting after the header.
    (void)expected_bytes; // retained for API clarity
